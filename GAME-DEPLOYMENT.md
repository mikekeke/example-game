...
Before we can start the game, we need to deploy contracts. One cumbersome thing with contracts, is that they are generated by `paima-engine` and then require some edits before they can be deployed. `example-game` directory contains examples of already edited contracts and scripts, and in most cases copy-pasting them to the correct location should just work. We will go through the process step by step. 
...
- Deploy two contracts. Here we will need to deploy two contracts to Ganache network (option 3). 
  - Set hex of private key if it is not there yet (`export PRIVATE_KEY=...`, w/o `0x` prefix)
  - Run `./deploy.sh` adn select option `3` for Ganache network
