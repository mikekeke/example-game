...
Before we can start the game, we need to deploy contracts. One cumbersome thing with contracts, is that they are generated by `paima-engine` and then require some edits before they can be deployed. `example-game` directory contains examples of already edited contracts and scripts, and in most cases copy-pasting them to correct location should just work. We will go through the process step by step. 
...
Before deploy, contract configs need to be adjusted. Most adjustments requires setting owner of the contract (which is the public key hash of desired account). With Ganache, it is possible to create network with known keys by setting known seed phrase. Current examples of edited contracts in [extra](./extra) directory have the "owner" set to the key derived from known seed phrase. The seed is
...
So if you start Ganache network with this seed phrase and use same `paima-engine` version (newer were not tested, but may ork as well), then files from `extra` directory could be copied to corresponding locations inside the `root/contracts` directory to overwrite existing ones. Files in `extra` directory also has Ganache network added to configs and deployment scripts, as files generated by `paima-engine` does not include Ganache network. But further explanation will cover in details what need to be configured or adjusted in all required "default" files in `root/ contracts` directory.

Lets start with L2 contract deployment - it is not too involved and requires minimal configuration.

We will need hex representations of public and private keys for deploy. For all contracts in this guide same single key pair will be used.
...
This contract will alow minting of "stateful" NFT that will track users achievements.

 - `cd` to `root/contracts/nft`
 - *(if required run `nvm use 16.20.0`)* run `npm i`
 - 3 files need to be edited here to enable current setup with Ganache (again, if you are using `known seed` and same version of `paima-engine`, it should be safe to copy over files from [extra/contracts/nft](./extra/contracts/nft/directory)
   ...
   - Run `./deploy.sh`. Here we will need to deploy two contracts to Ganache network (option 3). Setup can ask for hex of private key. Key can also be set to `PRIVATE_KEY` env variable (w/o `0x` prefix). 
    
      - Select option `1` (Deployment functionality), then `1` again (The Nft contract (Paima ERC721)).
      - Follow the suggestion at the end of terminal output: in `root` create `extensions.yml` and under `extensions:` add suggested data. E.g. of file content:
        ```
        extensions:
      ...
      - Run `./deploy.sh` gain. Again Select option `3` for Ganache, then option `1` for deploy, then `2` for "The NativeNftSale contract". Put a note somewhere with "Deployed contract addresses" data.
...
Now after L2 contract is deployed and game code and `middleware` packed, game can be launched.

Prerequisites:
  - Ganache is running and contracts are deployed
  - Docker to start database container
  - Browser with Metamask wallet extension. Current account is the account imported from private key hex of desired Ganache account
...
Follow this steps:
  ...
  - Open `example-game` and run `npm run frontend`. Frontend will be served on port `8080`.
